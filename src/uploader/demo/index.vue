<template>
  <demo-section>
    ...
    <demo-block :title="$t('preview')">
      <jy-uploader v-model="fileList" multiple accept="*" />
    </demo-block>

    <demo-block :title="$t('maxCount')">
      <jy-uploader v-model="fileList2" multiple :max-count="2" />
    </demo-block>

    <demo-block :title="$t('uploadStyle')">
      <jy-uploader>
        <jy-button type="primary" icon="photo">{{ this.$t('upload') }}</jy-button>
      </jy-uploader>
    </demo-block>

    <demo-block :title="$t('beforeRead')">
      <jy-uploader v-model="fileList3" :before-read="beforeRead" />
    </demo-block>
  </demo-section>
</template>
  ...
  data() {
    return {
      fileList: [
        ...
        { url: 'https://img.yzcdn.cn/jyt/tree.jpg' }
      ],
      fileList2: [{ url: 'https://img.yzcdn.cn/jyt/sand.jpg' }],
      fileList3: []
    };
  },
